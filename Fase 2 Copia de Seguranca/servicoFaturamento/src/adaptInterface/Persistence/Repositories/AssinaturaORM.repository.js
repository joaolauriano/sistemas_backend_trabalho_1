...
export class AssinaturaRepositoryORM extends IAssinaturaModelRepository{
  ...
  async buscarPorPlano(codPlano) {
    ...
    const resultados = await this.#assinaturaRep.find({
  ...
  static createFromObject({codigo,codPlano,codCli,inicioFidelidade,fimFidelidade,dataUltimoPagamento,custoFinal,descricao,status}){
    const hoje = new Date();
    const statusCalculado = status ?? (new Date(fimFidelidade) > hoje ? 'ATIVO' : 'CANCELADO');
    
    return new AssinaturaModel(codigo,codPlano,codCli,inicioFidelidade,fimFidelidade,dataUltimoPagamento,custoFinal,descricao,statusCalculado);
  }
}